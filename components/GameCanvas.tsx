...
const GameCanvas: React.FC<GameCanvasProps> = ({ level, initialScore, initialWeaponLevel, onGameOver, onLevelClear }) => {
  ...
  const drawPlayerPlane = (ctx: CanvasRenderingContext2D, p: Player) => {
    ...
    ctx.beginPath();
    ctx.ellipse(cx + 10, cy + 20, width/2, height/2, 0, 0, Math.PI*2);
    ctx.fill();
    ...
    wingGrad.addColorStop(1, '#1e3a8a');
    ctx.fillStyle = wingGrad;
    ctx.beginPath();
    ctx.moveTo(x, y + height * 0.45);
    ctx.lineTo(x + width, y + height * 0.45);
    ctx.lineTo(x + width * 0.9, y + height * 0.55);
    ...
    ctx.beginPath();
    ctx.moveTo(cx, y);
    ctx.bezierCurveTo(cx - 12, y + 5, cx - 10, y + height, cx, y + height);
  ...
  const drawEnemyPlane = (ctx: CanvasRenderingContext2D, e: Enemy) => {
    ...
    if (type === 'BOSS') {
      ...
      bossGrad.addColorStop(0.5, COLORS.ENEMY_BOSS);
      ...
      ctx.beginPath(); ctx.arc(cx + width*0.2, y + height*0.5, 10, 0, Math.PI*2); ctx.fill();
      drawExhaust(ctx, cx - 30, y + 10, false);
      drawExhaust(ctx, cx + 30, y + 10, false);
      ...
      ctx.fillStyle = COLORS[`ENEMY_${type}` as keyof typeof COLORS] as string;
      ctx.beginPath();
      ...
      ctx.closePath();
      ctx.fill();
      ctx.fillStyle = '#fff'; ctx.beginPath(); ctx.arc(x + 5, y + height*0.3, 3, 0, Math.PI*2); ctx.fill();
      ctx.beginPath(); ctx.arc(x + width - 5, y + height*0.3, 3, 0, Math.PI*2); ctx.fill();
  ...
  const spawnBoss = useCallback(() => {
    ...
    enemiesRef.current.push({
      ...
      speed: 0.5,
      type: 'BOSS',
      health: config.health * (1 + (level - 1) * 0.6),
      maxHealth: config.health * (1 + (level - 1) * 0.6),
      fireCooldown: config.fireRate,
      bulletType: level === 3 ? 'BOSS_BURST' : 'BOSS_CIRCLE'
    });
  }, [level]);
  ...
  const update = useCallback(() => {
    ...
    if (keysPressed.current[' '] && player.fireCooldown <= 0) {
      ...
      if (player.weaponLevel >= 4) {
        bulletsRef.current.push({ x: cx - 5, y: py - 5, width: 6, height: 18, speed: bSpeed * 1.1, damage: 2, isPlayerBullet: true, vx: 0, vy: -bSpeed * 1.1 });
      }
    ...
    if (Math.random() < 0.018 && !bossActiveRef.current) {
        ...
        enemiesRef.current.push({
          ...
          width: config.width,
          height: config.height,
          speed: config.speed + (level * 0.15),
          type: typeKey as any,
          health: config.health,
          maxHealth: config.health,
          fireCooldown: Math.random() * config.fireRate,
          bulletType: typeKey === 'BOMBER' ? 'SPREAD' : 'NORMAL'
        });
    }
    ...
    enemiesRef.current = enemiesRef.current.filter(e => {
      if (e.type === 'BOSS') {
        ...
        else e.x += Math.sin(frameCountRef.current / 80) * 1.2;

        if (e.fireCooldown <= 0) {
          const bcx = e.x + e.width / 2;
          const bcy = e.y + e.height - 20;
          const bulletCount = level === 1 ? 12 : 16;
        ...
        if (e.fireCooldown <= 0) {
          ...
          const ecy = e.y + e.height;
          const bSpd = 4;
          if (e.bulletType === 'NORMAL') {
            bulletsRef.current.push({ x: ecx - 2, y: ecy, width: 4, height: 12, speed: bSpd, damage: 10, isPlayerBullet: false, vx: 0, vy: bSpd });
          } else {
            [-1, 0, 1].forEach(vx => bulletsRef.current.push({ x: ecx - 2, y: ecy, width: 5, height: 12, speed: bSpd, damage: 10, isPlayerBullet: false, vx: vx * 1.0, vy: bSpd }));
          }
  ...
  }, [level, initialScore, onGameOver, onLevelClear, spawnBoss, tilt]);
  ...
  const draw = useCallback((ctx: CanvasRenderingContext2D) => {
    ...
    cloudsRef.current.forEach(c => {
      ctx.globalAlpha = c.opacity; ctx.fillStyle = '#fff';
    ...
    powerUpsRef.current.forEach(p => {
      ...
      ctx.fillText(p.type === 'WEAPON' ? 'W' : 'H', p.x + p.width/2, p.y + p.height/2 + 4);
    });
    ...
    const hpGrad = ctx.createLinearGradient(hpX, 0, hpX + barWidth, 0);
    hpGrad.addColorStop(0, '#ef4444'); hpGrad.addColorStop(1, '#10b981');
    ctx.fillStyle = hpGrad; ctx.fillRect(hpX, 25, barWidth * hpPct, 14);
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => { keysPressed.current[e.key] = true; keysPressed.current[' '] = true; };
    ...
    <canvas 
      ...
      onMouseMove={(e) => {
        ...
        if (rect) {
          ...
          setTilt((targetX - playerRef.current.x) * 0.05);
